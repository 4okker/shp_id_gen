    ...
    >
      ...
      >
        <v-card-title class="justify-space-between">
          ...
          <v-btn @click="remove(idea.id)" color="red" class="white--text">Удалить</v-btn>
        </v-card-title>
        ...
        <v-card-actions>
          <v-list-item class="grow">
            ...
            <v-row
              ...
              justify="end"
            >
              <v-btn fab x-small color="red lighten-1" @click="like(idea.id)">
                <v-icon color="white">
                  mdi-heart
                </v-icon>
              </v-btn>
...
<script>
  export default {
    ...
    methods: {
        ...
        remove(id) {
          this.$axios.post(`http://localhost:8000/api/idea/delete`, {
            id: id
          }, {
            headers: {
              Authorization: `Bearer ${this.$store.state.apiToken}`,
